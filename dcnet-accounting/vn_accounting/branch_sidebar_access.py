...
# Canonical "Workspace Sidebar" doc name is "VN Accounting" (see install.py
# _sync_workspace_sidebar / _create_desktop_icon). This constant used to be
# "Ke Toan VN" (a Vietnamese-diacritic variant), which never matches — it
# silently broke both get_sidebar_item_catalog() (always returned []) and
# apply_branch_sidebar_item_access() (bootinfo lookup by "ke toan vn" never
# found the "vn accounting" key that Frappe/boot.py actually populate) —
# i.e. branch-level sidebar restriction was a no-op for every user.
WORKSPACE_SIDEBAR_NAME = "VN Accounting"
