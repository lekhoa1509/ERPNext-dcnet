	...
	<div class="auto-flow">
		...
		<div v-else class="card progress-card">
			...
			<div v-if="isDone" class="result-panel">
				...
				<details v-if="rc.Failed" class="advanced failed-rows-details" @toggle="onFailedRowsToggle">
					<summary class="warn-summary">
						⚠️ {{ __("{0} dòng lỗi chưa vào sổ").replace("{0}", fmtN(rc.Failed)) }}
						<a :href="failedRowsUrl" target="_blank" @click.stop style="margin-left:8px;font-size:12px">↗ {{ __("Mở trang list") }}</a>
					</summary>
					<div v-if="failedRowsLoading" style="padding:8px;color:var(--text-muted,#888);font-size:13px">{{ __("Đang tải…") }}</div>
					<table v-else-if="failedRowsData.length" class="result-table failed-table">
						<thead><tr>
							<th>{{ __("Loại file") }}</th>
							<th>{{ __("Dòng") }}</th>
							<th>{{ __("Doctype") }}</th>
						...
						<tbody>
							<tr v-for="r in failedRowsData" :key="r.name">
								<td>{{ r.file_type }}</td>
								<td class="num">{{ r.row_index }}</td>
								<td>{{ r.target_doctype || "—" }}</td>
								<td class="err-msg">{{ r.error_message || "—" }}</td>
							</tr>
						</tbody>
					</table>
					<div v-else style="padding:8px;font-size:13px;color:var(--text-muted,#888)">{{ __("Không có dữ liệu.") }}</div>
				</details>
...
// ---------------------------------------------------------------- failed rows inline
const failedRowsData = ref([]);
const failedRowsLoading = ref(false);
async function onFailedRowsToggle(e) {
	if (!e.target.open || failedRowsData.value.length) return;
	failedRowsLoading.value = true;
	try {
		const res = await store.getFailedRows(200);
		failedRowsData.value = res?.rows || [];
	} catch (_) {
		failedRowsData.value = [];
	} finally {
		failedRowsLoading.value = false;
	}
}
...
.failed-rows-details summary.warn-summary {
	cursor: pointer; color: var(--orange-700, #b45309);
	font-size: 13px; font-weight: 500; padding: 4px 0; list-style: none;
}
.failed-rows-details summary.warn-summary::before { content: "▶ "; font-size: 10px; }
.failed-rows-details[open] summary.warn-summary::before { content: "▼ "; }
.failed-table { width: 100%; max-height: 320px; display: block; overflow-y: auto; }
.failed-table .err-msg { max-width: 400px; white-space: pre-wrap; word-break: break-word; font-size: 12px; color: var(--red-600, #dc2626); }
</style>
