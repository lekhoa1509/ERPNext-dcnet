...
const PIPELINE_API = "vn_accounting.misa_migration.api.pipeline";
...
export const useMisaStore = defineStore("misaMigration", {
	state: () => ({
		...
		loading: false,
		last_error: null,
		// Auto-pipeline (one-shot flow)
		pipeline: null,     // parsed pipeline_json from server
		seconds_since_update: null, // dead-job detection (api.pipeline.get_pipeline)
		row_counts: {},     // Misa Migration Row status → count (live)
		ob_posting_date: null,
	}),
	...
	getters: {
		...
		// A batch can be abandoned (deleted) when nothing was posted, OR after
		// it's been reversed via Undo (REVERSED keeps its historical
		// posted_docs_count but its docs are cancelled). In-flight / live
		// POSTED batches must go through Undo first.
		canDiscard(state) {
			return (
				!!state.batch_name &&
				!["POSTING", "REVERSING", "POSTED"].includes(state.status) &&
				((state.posted_docs_count || 0) === 0 || state.status === "REVERSED")
			);
		},
	},
	...
	actions: {
		...
		async resumeFromStorage() {
			...
			if (stored) {
				try {
					...
					// get_batch returns null when the batch was deleted — clear
					// the stale resume key + fall through to server discovery.
		...
		async detectFileTypes(filenames) {
			return _call(`${API}.detect_file_types`, {
				filenames: JSON.stringify(filenames),
			});
		},

		async startAutoPipeline() {
			if (!this.batch_name) throw new Error("no batch");
			const r = await _call(`${PIPELINE_API}.start_auto`, {
				batch_name: this.batch_name,
			});
			this.startPipelinePoll();
			return r;
		},

		async resumeAfterStuck() {
			if (!this.batch_name) throw new Error("no batch");
			const r = await _call(`${PIPELINE_API}.resume_after_stuck`, {
				batch_name: this.batch_name,
			});
			this.startPipelinePoll();
			return r;
		...
		async fetchPipeline() {
			...
			const data = await _call(`${PIPELINE_API}.get_pipeline`, {
				batch_name: this.batch_name,
			});
			if (data) {
				this.status = data.status ?? this.status;
				this.company = data.company ?? this.company;
				this.batch_title = data.batch_title ?? this.batch_title;
				this.total_rows = data.total_rows ?? this.total_rows;
				this.posted_docs_count = data.posted_docs_count ?? 0;
				this.failed_rows_count = data.failed_rows_count ?? 0;
				this.ob_posting_date = data.ob_posting_date ?? null;
				this.pipeline = data.pipeline ?? null;
				this.row_counts = data.row_counts ?? {};
				this.seconds_since_update = data.seconds_since_update ?? null;
			}
			return data;
		},

		// Poll the pipeline every 2.5s while it runs; realtime events come
		// in between polls but the poll is the reliability floor (realtime
		// can drop). Auto-stops on terminal/paused states.
		startPipelinePoll() {
			if (this._pipe_handle) return;
			const tick = async () => {
				if (!this.batch_name) return;
				try {
					await this.fetchPipeline();
					_update_tab_title(this, this._pipelineTabHint());
				} catch (e) { /* keep polling */ }
				const cur = this.pipeline?.current;
				const stages = this.pipeline?.stages || [];
				const failed = stages.some((s) => s.status === "failed");
				const blocked = stages.some((s) => s.status === "blocked");
				const allDone = stages.length && stages.every((s) => s.status === "done");
				if (failed || blocked || allDone || ["POSTED", "REVERSED"].includes(this.status) && allDone) {
					this.stopPipelinePoll();
				}
			};
			this._pipe_handle = setInterval(tick, 2500);
			tick();
		},
		stopPipelinePoll() {
			if (this._pipe_handle) {
				clearInterval(this._pipe_handle);
				this._pipe_handle = null;
			}
		},
		_pipelineTabHint() {
			const stages = this.pipeline?.stages || [];
			...
			return {
				pct: stages.length ? Math.round(done * 100 / stages.length) : 0,
				phase_hint: cur ? cur.label : this.status,
			};
		},
		...
		// Abandon the current batch (delete it server-side) and reset the
		// wizard to a fresh state. Only valid when nothing has been posted —
		// the server re-validates and throws otherwise. Clears the poller +
		// the localStorage resume key via setBatch(null).
		async discardBatch() {
			if (!this.batch_name) return;
			this.loading = true;
			try {
				await _call(`${API}.discard_batch`, { batch_name: this.batch_name });
				this.stopProgressPoll();
				this.setBatch(null);
			} finally {
				this.loading = false;
