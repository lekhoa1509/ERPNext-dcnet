...
def after_migrate() -> None:
    ...
    # MUST run before any seed that saves VN Accounting Settings — heals orphaned
    # Account links (e.g. renamed TK) that would otherwise fail _validate_links().
    _reconcile_settings_account_links()
    _seed_lcv_allocation_settings()
    _seed_period_closing_account_settings()
    _seed_bctc_mapping_templates()
...
def _ensure_desktop_icon() -> None:
    ...
    and the real key is 'vn accounting' (see boot.py:_claim_workspace_ownership,
    which reads bootinfo["workspace_sidebar_item"]["vn accounting"]).
    NOTE: this docstring previously said the key was 'ke toan vn' — that was
    the same stale-name bug fixed in branch_sidebar_access.WORKSPACE_SIDEBAR_NAME
    (2026-07-08); it never matched anything and was corrected here to avoid
    misleading future edits. Diacritics in label causes KeyError and the icon
    is hidden.

    link_type here is set to 'Workspace Sidebar' below, which does NOT match
    this docstring's older claim that desktop.js get_route() only handles
    'External' / 'Workspace' / 'DocType'. Unverified against current Frappe
    v16 desktop.js source (not vendored in this checkout) — if the Desktop
    Icon fails to route on click, check this first.
    """
    desired = {
        "label": "VN Accounting",
        "icon": "accounting",
...
def _seed_treasury_settings() -> None:
    ...
    if changed:
        ...
        settings.flags.ignore_mandatory = True  # fresh site: reqd fields backfill later
        settings.save()
        frappe.db.commit()
...
def _reconcile_settings_account_links() -> None:
    """Self-heal orphaned Account Link fields in VN Accounting Settings.

    An Account's `name` embeds its account_name ("154 - <name> - <abbr>"). If a
    TK is renamed after Settings was seeded, the stored Link goes orphan and ANY
    later VN Accounting Settings.save() fails _validate_links() — blocking
    migrate even when the failing field is unrelated to the change.

    We re-resolve each orphaned Link by its account_number (scoped to the
    company encoded in the orphan's abbr) and rewrite it via db.set_single_value
    (no validation). Idempotent — only touches links whose target is gone.
    """
    if not frappe.db.exists("DocType", "VN Accounting Settings"):
        return
    from vn_accounting.utils.account_resolver import _extract_account_number
    ...
    for df in meta.fields:
        ...
        if not value or frappe.db.exists("Account", value):
            continue  # blank or still valid
        number = _extract_account_number(value)
        if not number:
            continue
        # Scope to the company encoded in the orphan's abbr (last segment)
        abbr = value.split(" - ")[-1].strip()
        company = frappe.db.get_value("Company", {"abbr": abbr}, "name")
        filters = {"account_number": number, "is_group": 0}
        if company:
            filters["company"] = company
        fixed = frappe.db.get_value("Account", filters, "name")
        if fixed:
            frappe.db.set_single_value("VN Accounting Settings", df.fieldname, fixed)
...
def _seed_workspace_role_gate() -> None:
    ...
    # Fresh site (0 company → 0 account): the period-closing reqd fields
    # (pnl_account_911, retained_earnings…) are still empty at install time
    # — they get backfilled by the seeds on after_migrate / first Company
    # save. Don't let THIS save abort the whole app install.
