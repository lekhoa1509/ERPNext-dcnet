...
def _voucher_lookup_key(doc_name: str, company: str) -> str:
    """Misa voucher number for a migrated doc — strips the fresh-company
    naming prefix ``{abbr}-`` when present (see importers/_naming.py)."""
    import re as _re
...
def repost_se_gl_from_nkc(
    ...
    # Match both bare voucher names (PX2025...) and fresh-company namespaced
    # names ({abbr}-PX2025...).
    _abbr = frappe.get_cached_value("Company", company, "abbr") or ""
    prefix_clauses = " OR ".join(
        f"name LIKE '{p}%%' OR name LIKE '{_abbr}-{p}%%'" for p in SE_PREFIXES
    )
    ...
    if batch_name:
        ...
        se_rows = [
            s for s in se_rows
            if s["name"] in batch_vnos
            or _voucher_lookup_key(s["name"], company) in batch_vnos
        ]
    else:
    ...
    for se in se_rows:
        # NKC lookup uses the bare Misa voucher number; GL delete/insert must
        # keep the REAL doc name (possibly '{abbr}-' namespaced).
        vno = se["name"]
        misa_vno = _voucher_lookup_key(se["name"], company)
        legs = _nkc_legs_for_voucher(batch_name, misa_vno)
        if not legs:
            continue
