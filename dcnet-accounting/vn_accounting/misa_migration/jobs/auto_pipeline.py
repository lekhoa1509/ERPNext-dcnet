...
def _stage_masters(batch_name: str, pl: dict[str, Any]) -> None:
    ...
    def _heartbeat(ft: str, counts: dict) -> None:
        """Touch batch.modified after each file-type so watchdog stays quiet."""
        posted = counts.get("posted", 0)
        exists = counts.get("exists", 0)
        failed = counts.get("failed", 0)
        pl["activity"] = _(
            "Đang cài: {0} · {1} tạo mới, {2} đã có, {3} lỗi"
        ).format(ft, posted, exists, failed)
        _save_pipeline(batch_name, pl)

    res = install_misa_coa(batch_name, _heartbeat=_heartbeat)
...
def _stage_post(batch_name: str, pl: dict[str, Any]) -> None:
    ...
    with _st.lock_for_batch(batch):
        batch = frappe.get_doc("Misa Migration Batch", batch_name)
        if batch.status == _st.REVIEWED:
            _st.transition(batch, _st.POSTING, reason="auto_pipeline bulk post")
            batch.save(ignore_permissions=True)
            frappe.db.commit()
        elif batch.status != _st.POSTING:
            # Batch is in an unexpected state (e.g. already POSTED or REVERSED).
            # Do not overwrite — surface a clear error.
            frappe.throw(
                _("Không thể post ở trạng thái {0} — phải là REVIEWED hoặc POSTING.").format(
                    batch.status)
            )
        # If already POSTING (worker crashed without setting STUCK, now
        # resuming via resume_after_stuck → REVIEWED path bypassed), allow
        # re-entry so run_bulk_pump_full can continue idempotently.

    def _heartbeat(msg: str) -> None:
        pl["activity"] = _("Đang ghi sổ (bulk): {0}").format(msg)
        _save_pipeline(batch_name, pl)
