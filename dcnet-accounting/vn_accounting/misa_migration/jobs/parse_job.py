...
# Department multi-sheet support — Danh_sach_co_cau_to_chuc_theo_chi_nhanh.xlsx
# ships Công ty + Chi nhánh + one "PB - <chi nhánh>" sheet per branch. Every
# sheet except "Công ty" (company already exists in ERPNext, not re-imported
# here) feeds Department rows. See importers/department.py for how the
# "_sheet" tag drives is_group / parent_department per row.
_DEPARTMENT_SKIP_SHEETS = {"công ty"}
_DEPARTMENT_BRANCH_MASTER_SHEET = "chi nhánh"


def _iter_department_rows(wb) -> Iterator[dict]:
    """Yield one payload dict per data row across all Department sheets.

    Sheet order: "Chi nhánh" is forced first (whatever its position in the
    workbook) so its rows get the lowest row_index and post_row creates the
    branch Departments before any per-branch "PB - ..." sheet is posted —
    DepartmentImporter.build_doc resolves branch parents by DB lookup at
    post time, which only succeeds once the branch already exists.
    """
    sheet_names = [s for s in wb.sheetnames if s.strip().lower() not in _DEPARTMENT_SKIP_SHEETS]
    sheet_names.sort(key=lambda s: 0 if s.strip().lower() == _DEPARTMENT_BRANCH_MASTER_SHEET else 1)
    for sheet_name in sheet_names:
        ...
        for idx, row in enumerate(row_iter):
            ...
            if idx >= 15:
                break
        headers, n_skipped = _find_header_row(iter(chunk))
        if not headers:
            continue
        for r in chunk[n_skipped:]:
            payload = _row_to_dict(r, headers)
            if payload:
...
def _bulk_insert_rows(rows: list[dict]) -> int:
    ...
    for r in rows:
        flat.extend([
            # length=20: 16^20 keyspace. length=10 collided for real on a
            # 105k-row batch (E2E vòng 3 — 1 trùng key giết cả chunk INSERT
            # 500 dòng → file Failed → pipeline STUCK).
            frappe.generate_hash(length=20), now, now, user, user,
            0, 0,
...
def parse_file(batch_name: str, file_row_name: str) -> dict:
    ...
    try:
        if file_row.file_type == "SCT":
            pass  # sectioned layout, max_row inaccurate
        elif file_row.file_type == "Department":
            from openpyxl import load_workbook as _lw
            _wb = _lw(file_path, read_only=True, data_only=True)
            for _sn in _wb.sheetnames:
                if _sn.strip().lower() in _DEPARTMENT_SKIP_SHEETS:
                    continue
                estimated_total += max((_wb[_sn].max_row or 0) - 3, 0)
            _wb.close()
        else:
            from openpyxl import load_workbook as _lw
            _wb = _lw(file_path, read_only=True, data_only=True)
        ...
        estimated_total = 0
    # Restart safety: a prior run of this file may have died mid-insert
    # (bench restart, OOM) leaving partial rows. Re-parsing would APPEND
    # duplicates on top. Clear this file's staging rows first. Guard: only
    # when the batch has exactly ONE file of this type — rows carry
    # (batch, file_type) but no per-file link, so with 2+ same-type files
    # we'd wipe a sibling's rows (log + skip cleanup in that rare case).
    same_type = [f for f in batch.files if f.file_type == file_row.file_type]
    if len(same_type) == 1:
        frappe.db.sql(
            "DELETE FROM `tabMisa Migration Row` WHERE batch=%s AND file_type=%s",
            (batch_name, file_row.file_type),
        )
        frappe.db.commit()
    elif frappe.db.count("Misa Migration Row",
                         {"batch": batch_name, "file_type": file_row.file_type}):
        frappe.log_error(
            title="Misa parse_file: skip stale-row cleanup",
            message=f"{batch_name}/{file_row.file_type}: {len(same_type)} files share "
                    "this type — cannot safely clear staging rows; duplicates possible "
                    "if a prior run of this file died mid-insert.",
        )
    _update_file_row(
        file_row_name,
    ...
    try:
        ...
        if file_row.file_type == "SCT":
            ...
            # Multi-sheet org-structure workbook (Chi nhánh + per-branch
            # phòng ban sheets) — see _iter_department_rows. Every other
            # file_type is exactly one flat sheet, handled by the generic
            # branch below.
            wb = load_workbook(file_path, read_only=True, data_only=True)
            row_index = 0
            for payload in _iter_department_rows(wb):
                row_index += 1
                cols = max(cols, len(payload))
                buffer.append({
                    "batch": batch_name,
                    "file_type": file_row.file_type,
                    "entity_type": entity_type,
...
def _parse_files_parallel(batch_name: str, files_to_parse: list[tuple[str, str]]) -> None:
    ...
    # Reset state BEFORE dispatch: a re-parse of a previously-Failed file
    # would otherwise race the poll below — 'Failed' counts as terminal, so
    # the poll returns instantly with the STALE Failed status while the
    # fresh job is still queued (resume bug, E2E vòng 3b: parse_batch
    # raised on last run's error 1 giây sau khi resume).
    for fname, _ftype in files_to_parse:
        frappe.db.sql(
            """UPDATE `tabMisa Migration File`
               SET parse_status='Pending', parse_error=NULL, modified=NOW()
               WHERE name=%s""",
            (fname,),
        )
    frappe.db.commit()
    ...
    while _t.time() < deadline:
        ...
        if pending == 0:
            return
        # Heartbeat every 30s: bump batch.modified so the UI dead-job detector
        # (3-minute threshold in AutoFlow.vue) doesn't fire during a long parse.
        if _t.time() - last_heartbeat > 30:
            done = n_total - pending
            frappe.db.sql(
                "UPDATE `tabMisa Migration Batch` SET modified=NOW(6) WHERE name=%s",
                (batch_name,),
            )
            frappe.db.commit()
            _publish(batch_name, {"activity": _("Đang phân tích file… {0}/{1} xong").format(done, n_total)})
            last_heartbeat = _t.time()
        _t.sleep(2)
