...
def run_preview(batch_name: str) -> dict:
    ...
    for ft in file_types_in_batch:
        ...
        if not importer:
            continue
        # PERF: filter by file_type in SQL. The old shape iterated EVERY
        # New row of the batch per file_type (O(N×M): a 105k-row batch ×
        # 31 types ≈ 3.2M frappe.get_doc loads → preview crawled at ~10
        # rows/s and the whole pipeline looked hung at "Phân tích file").
        names = frappe.db.sql_list(
            """SELECT name FROM `tabMisa Migration Row`
               WHERE batch=%s AND status='New' AND file_type=%s
               ORDER BY
                 CASE WHEN file_type='Account'
                      THEN CHAR_LENGTH(COALESCE(JSON_UNQUOTE(JSON_EXTRACT(parsed_payload, '$._tk')), ''))
                      ELSE 0
