...
class BankAccountImporter(BaseImporter):
    ...
    def _find_bank_by_name_or_bank_name(self, target: str) -> str | None:
        """Return Bank.name if any Bank matches target by name or bank_name.

        Two passes:
          1. Exact case-insensitive match.
          2. Word-prefix match: alias target starts with the bank's name
             and the next char is a separator (e.g. "MB Bank" matches "MB").
        """
        target_lower = target.strip().lower()
        for b in self._banks:
            if (b.name or "").strip().lower() == target_lower:
                return b.name
            if (b.bank_name or "").strip().lower() == target_lower:
                return b.name
        # Prefix pass: "MB Bank" should match a bank whose name is "MB"
        for b in self._banks:
            for candidate in (
                (b.name or "").strip().lower(),
                (b.bank_name or "").strip().lower(),
            ):
                if not candidate or len(candidate) < 2:
                    continue
                if target_lower == candidate:
                    return b.name
                if target_lower.startswith(candidate) and target_lower[len(candidate)] in (" ", "-", "_"):
                    return b.name
                if candidate.startswith(target_lower) and candidate[len(target_lower)] in (" ", "-", "_"):
                    return b.name
        return None
    ...
    def _resolve_bank(self, misa_long_name: str) -> str | None:
        ...
        if alias_target:
            match = self._find_bank_by_name_or_bank_name(alias_target)
            if match:
                return match
        ...
        for cut in range(len(normalized), 5, -1):
            ...
            if head in self._BANK_NAME_ALIASES:
                ...
                match = self._find_bank_by_name_or_bank_name(tgt)
                if match:
                    return match
