...
def check_tk_mapping(
    ...
    # Company-verified resolution — a mapping entry pointing at ANOTHER
    # company's account (site-wide Single, stale from a prior migration)
    # must count as unmapped here, otherwise preflight passes and every
    # voucher fails at insert ("Tài khoản X không thuộc về công ty Y").
    ...
    for v in vouchers:
        for leg in v.get("legs") or []:
            ...
            if not tk:
                continue
            if not resolve_account(tk, mapping, company):
                unmapped.add(tk)
...
def check_no_existing_posted(
    ...
    """9. No existing SUBMITTED ERPNext doc with the same (namespaced) name.

    Checks all 5 target DocTypes (SI, PI, PE, JE, SE — PR covered by PI).

    The handlers name migrated docs ``{company_abbr}-{voucher_no}`` so two
    companies' overlapping Misa voucher numbers don't collide. This check
    therefore builds the company-namespaced names and matches on those —
    another company's identical voucher numbers carry a different name and
    no longer false-block a brand-new-company migration.
    ...
    # Company-namespaced names — matching these auto-scopes to the target
    # company (another company's docs have a different abbr prefix).
    names = [migrated_doc_name(company, vn) for vn in voucher_nos]
    misa_imported: list[str] = []   # created by a previous Misa run → safe to skip
    manual_dupes: list[str] = []    # NOT from Misa → genuine collision risk
    for dt in target_doctypes:
        has_misa_field = frappe.db.has_column(dt, "misa_voucher_no")
        select_fields = "name, docstatus" + (", misa_voucher_no" if has_misa_field else "")
        rows = frappe.db.sql(
            f"""
            SELECT {select_fields} FROM `tab{dt}`
        ...
        for r in rows:
            label = f"{dt} {r['name']} đã tồn tại (docstatus=1, đã submit)"
            if has_misa_field and r.get("misa_voucher_no"):
                # Previously imported by Misa — bulk_pump will skip it.
                misa_imported.append(label + " — sẽ bỏ qua khi chạy lại")
            else:
                manual_dupes.append(label)
    # Only BLOCK when there are manually-created docs with the same name —
    # those indicate a genuine collision. Previously-imported docs are safe
    # to skip (bulk_pump is idempotent on submitted docs) → downgrade to warn.
    all_issues = manual_dupes + misa_imported
    has_block = bool(manual_dupes)
    label = (
        "Voucher trùng tên (tạo thủ công)" if has_block
        else "Voucher đã import lần trước — sẽ bỏ qua"
    )
    return {
        "name": "no_existing_posted",
        "label": label,
        "level": "block" if has_block else "warn",
...
def run_preflight(
    ...
    checks.append(check_no_existing_posted(vouchers, company))
