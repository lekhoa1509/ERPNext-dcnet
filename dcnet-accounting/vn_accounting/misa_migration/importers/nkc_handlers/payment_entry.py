...
# Account resolution + caches moved to _account_lookup (canonical,
# COMPANY-VERIFIED — see that module's docstring for the cross-company
# mapping leak it fixes). Re-exported here because sibling handlers
# (SI/PI/JE/PR) import these names from payment_entry.
from vn_accounting.misa_migration.importers.nkc_handlers._account_lookup import (  # noqa: F401
    _ACCOUNT_BY_NUMBER_CACHE,
    _ACCOUNT_META_CACHE,
    is_group_cached as _is_group_cached,
    resolve_account,
    reset_account_caches as _reset_account_caches,
    warm_account_cache_for_company,
)


def _warm_account_cache_for_company(company: str) -> None:
    """Delegates to _account_lookup (kept for orchestrator import compat)."""
    warm_account_cache_for_company(company)


def reset_perf_caches() -> None:
    """Called by orchestrator between batches (or at startup) to clear
    stale per-request caches."""
    _reset_account_caches()
...
def _create_simple_pe(
    ...
    # Company-namespaced doc name (avoids cross-company voucher_no collision).
    target_name = migrated_doc_name(company, voucher_no)
    if frappe.db.exists("Payment Entry", target_name):
