...
@frappe.whitelist()
def get_batch(batch_name: str) -> dict[str, Any] | None:
    """Return current state of a batch for store hydration.

    Returns None (HTTP 200) when the batch doesn't exist instead of throwing
    — the hub resumes the last batch name from localStorage and polls it, so a
    deleted/stale name must NOT 417 (a thrown error is logged to the browser
    console and can't be suppressed). The JS treats null as "not found" and
    clears the stale resume key.
    """
...
def find_active_batch(company: str | None = None) -> dict[str, Any] | None:
    """Find a genuinely in-flight Misa Migration Batch for cross-session resume.

    Only returns batches that are RUNNING (POSTING/REVERSING) or need
    recovery (STUCK) — these must never be lost if the operator navigates
    away or the worker dies. "Parked" states (DRAFT/UPLOADED/PARSED/
    REVIEWED) are deliberately EXCLUDED: they represent a paused, not
    running, migration and should resume only via localStorage in the SAME
    browser — otherwise a fresh session (or a different operator opening the
    hub) would inherit a stale parked batch and land mid-wizard with no way
    out. A parked batch is always reachable via "Lịch sử" (batch list).

    When `company` is None, looks across all companies the user can access.

    Returns serialized batch dict or None if no in-flight batch found.
    """
    active_statuses = (st.POSTING, st.REVERSING, st.STUCK)
...
@frappe.whitelist()
def discard_batch(batch_name: str) -> dict[str, Any]:
    """Abandon a never-posted batch — the UI escape hatch ("Bỏ batch này").

    Deletes the batch and its Misa Migration Row rows so it stops blocking
    new batches (st.ACTIVE_STATUSES admission guard) and stops resurfacing.

    Safe ONLY when nothing has been posted to ERPNext: status not in
    POSTING/POSTED/REVERSING AND posted_docs_count == 0. A batch that
    created GL docs must be reversed via Undo first — discarding it would
    orphan those docs. The batch's own on_trash guard normally blocks
    deleting a PARSED/REVIEWED batch; this endpoint is the sanctioned
    escape, so it deletes with ignore_on_trash AFTER the safety check.
    Delete permission on Misa Migration Batch is still enforced.
    """
    if not batch_name or not frappe.db.exists(DOCTYPE_BATCH, batch_name):
        frappe.throw(_("Batch không tồn tại: {0}").format(batch_name))

    def _blocked(status, posted):
        if status in (st.POSTING, st.REVERSING):
            return _("Batch đang chạy ({0}) — đợi hoàn tất rồi mới bỏ được.").format(status)
        # DRAFT / REVERSED always OK (never created / already undone). Only a
        # live POSTED batch needs Undo first.
        if status not in (st.DRAFT, st.REVERSED) and (status == st.POSTED or posted > 0):
            return _("Batch đã tạo {0} chứng từ vào ERPNext — phải Hoàn tác (Undo) trước khi bỏ.").format(posted)
        return None

    batch = frappe.get_doc(DOCTYPE_BATCH, batch_name)
    msg = _blocked(batch.status, frappe.cint(batch.posted_docs_count))
    if msg:
        frappe.throw(msg)

    with st.lock_for_batch(batch):
        # Re-fetch + re-check inside the lock — a concurrent post may have
        # advanced the batch between the check above and acquiring the lock.
        batch = frappe.get_doc(DOCTYPE_BATCH, batch_name)
        msg = _blocked(batch.status, frappe.cint(batch.posted_docs_count))
        if msg:
            frappe.throw(msg)
        n_rows = frappe.db.count("Misa Migration Row", {"batch": batch_name})
        # Rows link to the batch (not a child table) → delete them first or
        # the batch delete hits LinkExistsError. Delete in COMMITTED CHUNKS so
        # a large batch (10k+ rows) doesn't hold one giant row lock — that lock
        # otherwise wait-times-out against the hub's realtime poll / a second
        # click ("Lock wait timeout exceeded").
        CHUNK = 2000
        while True:
            ids = frappe.db.sql_list(
                "SELECT name FROM `tabMisa Migration Row` WHERE batch=%s LIMIT %s",
                (batch_name, CHUNK),
            )
            if not ids:
                break
            ph = ",".join(["%s"] * len(ids))
            frappe.db.sql(
                f"DELETE FROM `tabMisa Migration Row` WHERE name IN ({ph})",
                tuple(ids),
            )
            frappe.db.commit()
        # Rows gone → delete_doc on the now-empty batch is fast + lock-free
        # (skips on_trash via ignore_on_trash).
        frappe.delete_doc(
            DOCTYPE_BATCH, batch_name,
            force=True, ignore_on_trash=True, delete_permanently=True,
        )
    frappe.db.commit()
    return {"discarded": batch_name, "deleted_rows": n_rows}
...
@frappe.whitelist()
def detect_file_types(filenames) -> dict[str, str]:
    """Client preview: filename → detected file_type for a list of names.

    The auto-flow UI shows detected types BEFORE creating the batch so the
    operator can spot an 'Unknown' early. Same regex table attach_file
    uses — single source of truth."""
    import json as _json
    if isinstance(filenames, str):
        filenames = _json.loads(filenames or "[]")
    return {fn: detect_file_type_from_name(fn) for fn in (filenames or [])}
