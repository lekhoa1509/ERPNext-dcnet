...
def resume_after_stuck(batch_name: str) -> dict[str, Any]:
    """Recover a STUCK (or orphaned POSTING) batch and re-run the pipeline.

    STUCK → PARSED/UPLOADED then re-enqueue, as before.
    POSTING → treated as STUCK: the worker died hard without the exception
    handler firing (SIGKILL / bench restart mid-bulk-pump). Reset to PARSED
    so the pipeline re-runs masters + post idempotently.
    """
    if not batch_name or not frappe.db.exists(DOCTYPE_BATCH, batch_name):
        frappe.throw(_("Batch không tồn tại: {0}").format(batch_name))
    batch = frappe.get_doc(DOCTYPE_BATCH, batch_name)
    if batch.status not in (st.STUCK, st.POSTING):
        frappe.throw(_("Batch không ở trạng thái STUCK (đang là {0}).").format(batch.status))
    ...
    with st.lock_for_batch(batch):
        batch = frappe.get_doc(DOCTYPE_BATCH, batch_name)
        # POSTING is not in STUCK's allowed sources; bridge via STUCK first.
        if batch.status == st.POSTING:
            st.transition(batch, st.STUCK, reason="resume_after_stuck: orphaned POSTING → STUCK")
