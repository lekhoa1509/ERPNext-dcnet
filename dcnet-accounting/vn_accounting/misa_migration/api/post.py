...
def ensure_migration_custom_fields() -> list[str]:
    """Create the Custom Fields the migration pipeline writes/reads.

    Mirrors the vn_accounting fixture definitions (custom_field.json) for
    the misa_* fields only. Needed because fixture sync is not guaranteed
    on a fresh site (sync aborts mid-file when a record targets a DocType
    of an uninstalled app, e.g. Expense Claim without HRMS). Idempotent:
    create_custom_fields skips existing fields; absent DocTypes are
    filtered out first.
    """
    ...
    def _cf(label: str, fieldname: str, insert_after: str) -> dict:
        return {
            "label": label, "fieldname": fieldname, "fieldtype": "Data",
            "insert_after": insert_after, "no_copy": 1, "search_index": 1,
        }

    spec = {
        "Customer": [_cf("Mã Misa (Khách hàng)", "misa_party_code", "naming_series")],
        "Supplier": [_cf("Mã Misa (NCC)", "misa_party_code", "naming_series")],
        "Employee": [_cf("Mã Misa (Nhân viên)", "misa_employee_code", "naming_series")],
        "Item": [_cf("Mã Misa (Hàng hóa)", "misa_item_code", "item_code")],
        "Journal Entry": [_cf("Số chứng từ Misa", "misa_voucher_no", "naming_series")],
        "Payment Entry": [_cf("Số chứng từ Misa", "misa_voucher_no", "naming_series")],
        "Purchase Invoice": [_cf("Số chứng từ Misa", "misa_voucher_no", "naming_series")],
        "Purchase Receipt": [_cf("Số chứng từ Misa", "misa_voucher_no", "naming_series")],
        "Sales Invoice": [_cf("Số chứng từ Misa", "misa_voucher_no", "naming_series")],
        "Stock Entry": [_cf("Số chứng từ Misa", "misa_voucher_no", "naming_series")],
    }
    created: list[str] = []
    for dt in list(spec):
        if not frappe.db.exists("DocType", dt):
            ...
            continue
        for f in spec[dt]:
            if not frappe.db.has_column(dt, f["fieldname"]):
                created.append(f"{dt}-{f['fieldname']}")
    if spec:
        create_custom_fields(spec, ignore_validate=True, update=False)
        frappe.db.commit()
    return created


@frappe.whitelist()
def install_misa_coa(batch_name: str, _heartbeat=None) -> dict:
    ...
    # Step -1: ensure the migration's own Custom Fields (misa_voucher_no &
    # friends) exist. Fixture sync can abort mid-file on a fresh site when
    # custom_field.json hits a record for an uninstalled app's DocType
    # (Expense Claim/HRMS) — every record AFTER it is silently skipped →
    # bulk INSERT dies with "Unknown column 'misa_voucher_no'". Idempotent;
    # skips DocTypes absent on the site.
    ensure_migration_custom_fields()
    ...
    for ft in _BOOTSTRAP_FILE_TYPES:
        ...
        if _heartbeat:
            try:
                _heartbeat(ft, importer.counts)
            except Exception:
                pass

    # Reduced file sets (e.g. monthly 1-2026: NKC + bảng kê + TB only, no
    # Danh_sach_he_thong_tai_khoan) reference sub-accounts (1121.20, 11215…),
    # warehouses, items and parties that no Phase 1/2/3 file creates. Derive
    # them from the batch's own rows. Idempotent; mostly a no-op for full sets.
    try:
        from vn_accounting.misa_migration.scripts.derive_masters_from_batch import (
            derive_masters_for_batch,
        )
        summary["__derived_masters__"] = derive_masters_for_batch(batch_name)
        frappe.db.commit()
    except Exception as exc:
        frappe.log_error(title="Misa derive_masters_for_batch failed",
                         message=str(exc))

    # Fiscal Years: a fresh site's wizard creates only the CURRENT year —
    # migrating last year's data (OB on 2024-12-31, vouchers through 2025)
    # then trips FiscalYearError on every ORM-built doc. Ensure one FY per
    # calendar year spanned by the batch's parsed rows (data-driven).
    try:
        # NB: rows store source data in raw_payload (parsed_payload is NULL
        # on the bulk path) — read Misa's "Ngày hạch toán" key directly.
        # REGEXP guard: footer rows carry text ("Tổng cộng") in that key.
        yr_row = frappe.db.sql(
            """SELECT MIN(JSON_UNQUOTE(JSON_EXTRACT(raw_payload, '$."Ngày hạch toán"'))),
                      MAX(JSON_UNQUOTE(JSON_EXTRACT(raw_payload, '$."Ngày hạch toán"')))
               FROM `tabMisa Migration Row`
        ...
        if yr_row and yr_row[0][0]:
            y_min = int(str(yr_row[0][0])[:4]) - 1  # OB falls in the prior year
            y_max = int(str(yr_row[0][1])[:4])
            for y in range(y_min, y_max + 1):
                if not frappe.db.exists("Fiscal Year", str(y)):
                    fy = frappe.get_doc({
                        "doctype": "Fiscal Year",
                        "year": str(y),
                    ...
                    fy.insert()
                    summary.setdefault("__fiscal_years__", []).append(str(y))
            frappe.db.commit()
    except Exception as exc:
        frappe.log_error(title="Misa ensure fiscal years failed", message=str(exc))

    # Safety net: critical account_type bits the GL engine HARD-requires
    # regardless of which path created the account (TT99 bootstrap, Misa
    # importer, manual). Runs before any GL exists, so the type change is
    # always allowed. Idempotent — only fills blanks.
    type_fixes = ensure_critical_account_types(company)
    if type_fixes:
        summary["__account_type_fixes__"] = type_fixes
        frappe.db.commit()

    # Company default-account repair (override): the Misa Account import may
    # wipe-and-replace the skeleton chart the Company was created with —
    # Company.default_inventory_account & friends then point at DELETED
    # account names ("tồn kho trong tay - XX"). ERPNext's warehouse-account
    # fallback chain dies on that ghost ref → every Stock Entry submit fails
    # ("Warehouse X is not linked to any account...", 1.111 SE drafts on the
    # 2nd fresh-company E2E). refresh re-resolves every default by TT99
    # account number, longest-leaf first.
    from vn_accounting.misa_migration.bulk_pump.coa_bootstrap import (
        refresh_company_default_accounts,
    )
    try:
        summary["__company_defaults__"] = refresh_company_default_accounts(company)
    except Exception as exc:
        frappe.log_error(title="Misa refresh_company_default_accounts failed",
                         message=str(exc))

    # Warehouses created by the Misa importer carry no `account` — point
    # them at the inventory leaf so perpetual-inventory SLE→GL resolution
    # never falls back to (possibly ghost) company defaults.
    inv_leaf = frappe.db.get_value(
        "Account",
        {"company": company, "account_number": ["like", "1561%"], "is_group": 0},
        "name", order_by="account_number",
    ) or frappe.db.get_value(
        "Account",
        {"company": company, "account_number": ["like", "156%"], "is_group": 0},
        "name", order_by="account_number",
    )
    if inv_leaf:
        frappe.db.sql(
            """UPDATE `tabWarehouse` SET account=%s
...
# Minimal, production-proven set (mirrors the live DCNET company):
# (account_number prefix, required type). Receivable/Payable are the two
# the GL engine refuses to post without (SI debit_to / PI credit_to);
# Tax/Cash keep taxes + cash book features working.
_CRITICAL_TYPE_BY_NUMBER: tuple[tuple[str, str], ...] = (
    ("131", "Receivable"),
    ("331", "Payable"),
    ("1331", "Tax"),
    ("33311", "Tax"),
...
def ensure_critical_account_types(company: str) -> dict[str, str]:
    """Fill blank account_type on critical leaves. Returns {account: type}."""
    fixed: dict[str, str] = {}
    for prefix, atype in _CRITICAL_TYPE_BY_NUMBER:
        rows = frappe.db.sql(
        ...
        for (name,) in rows:
            frappe.db.set_value("Account", name, "account_type", atype,
