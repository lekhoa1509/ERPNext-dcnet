...
│ - Contract Item             │  ref   │   (contract_ref → Contract) │
│ - Billing Schedule          │        │ - PAKD Item                 │
│ - Contract Revision         │        │ - PAKD Commission Line      │
│ - Contract Template         │        │ - Commission Rule Template  │
│                             │        │ - PAKD Settings             │
│ Owns:                       │        │ Owns:                       │
│ - Contract lifecycle        │        │ - Commission policy         │
│ - Billing schedule gen      │        │ - Commission calc + posting │
│ - Auto-invoice scheduler    │        │ - Chi phí ngoài (MS/AC/GPVT)│
│ - Payment Entry hook        │        │ - Approval workflow         │
│ - Overdue scheduler         │        │ - Mẫu 01/02/03 print format │
│ - Shadow SO sync            │        │                             │
│ - Revision chain            │        │                             │
│ - Contract print format     │        │                             │
└─────────────────────────────┘        └─────────────────────────────┘
         ▲                                        │
         │ doc_event: billing_schedule.paid       │
         └────────────────────────────────────────┘
...
## 4. Quyết định: Option D — Contract-first, shadow SO

Với hợp đồng recurring (không vật tư): **Contract thuần**. Không có Sales Order nào. Billing schedule sinh Sales Invoice trực tiếp.

Với hợp đồng one-off có vật tư (VTTB, thi công): **Contract + shadow Sales Order**. Khi Contract Activate, hook tự tạo một SO ẩn — user không thấy, không sửa được — làm cánh tay nối dài vào hệ thống kho ERPNext (Delivery Note, Stock Entry, Batch, Serial, tax template). Kế toán vẫn tạo DN + SI từ SO như bình thường. Sync 1 chiều Contract → SO qua doc_event, cascade delete khi Contract xoá.
...
**Q: Mẫu 01/02/03 Excel khác nhau nhiều. Một PAKD DocType có cover cả 3 được không?**
Có. Enum `pakd_type` (Recurring Telecom / Monthly FTTH Rollup / One-off Sale) quyết định print format nào render. Approval workflow và commission calc dùng chung. Đã verify 3 file Excel thực tế có cùng structure nghiệp vụ, khác nhau chủ yếu ở layout.

**Q: Vì sao PAKD không phải là child table của Contract?**
Vì PAKD có vòng đời và approval workflow độc lập. Một Contract có thể có 0 PAKD (chưa trình), 1 PAKD, hoặc nhiều PAKD (sau mỗi revision). Child table không mô hình nổi quan hệ này. Hơn nữa Mẫu 02 là quan hệ 1 PAKD → N Contracts — child table chỉ đi theo hướng ngược.
