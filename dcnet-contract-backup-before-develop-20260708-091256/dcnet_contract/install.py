...
def _get_contract_settings():
	"""Return settings doc when the Single DocType is fully available.

	During `bench install-app`, Frappe can invoke `after_install` before the
	DocType controller is resolvable from cache. In that window, trying to load
	the Single via `frappe.get_doc()` raises an import error even though the app
	itself is already being installed. We let `after_migrate` finish the seeding
	once the DocType registry is stable.
	"""
	if not frappe.db.exists("DocType", "DCNet Contract Settings"):
		return None

	try:
		return frappe.get_doc("DCNet Contract Settings", "DCNet Contract Settings")
	except ImportError as exc:
		if "dcnet_contract_settings" in str(exc):
			return None
		raise
...
def _ensure_required_master_data():
	"""Seed tiny ERPNext masters required by contract template fixtures.

	A freshly recreated dev site can skip ERPNext's setup wizard, so stock
	defaults such as UOM "Nos" may not exist yet. Contract template child rows
	default to "Nos"; create it before inserting templates so migrate remains
	idempotent on reset sites.
	"""
...
def _sync_workspace_sidebar():
	...
	items = _dedupe_sidebar_items(data.get("items", []))
...
def _dedupe_sidebar_items(items):
	"""Remove duplicate sidebar items while preserving the first occurrence."""
	meta_fields = {
		"doctype",
		"name",
		"parent",
	...
	for item in items:
		cleaned = {k: v for k, v in item.items() if k not in meta_fields}
...
def _ensure_desktop_icon():
	...
	if frappe.db.exists("Desktop Icon", {"label": label}):
		name = frappe.db.get_value("Desktop Icon", {"label": label}, "name")
