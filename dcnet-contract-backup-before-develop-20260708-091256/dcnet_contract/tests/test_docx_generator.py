...
class TestFillDocx(unittest.TestCase):
    ...
    def test_items_table_without_item_stt_still_replaces_item_placeholders(self):
        """Templates with fixed STT cell should still be treated as item rows."""
        from docx import Document
        ...
        table.rows[2].cells[1].text = "Tong cong"
        ...
        items = [
            {"item_stt": "1", "item_label": "Kenh MPLS", "item_qty": "1", "item_uom": "kenh", "item_price": "7,000,000", "item_amount": "7,000,000"},
        ]
        result_path = _fill_docx(tmp_in.name, {}, items)
        ...
        row1_text = " ".join(cell.text for cell in result_table.rows[1].cells)
        self.assertIn("Kenh MPLS", row1_text)
        self.assertNotIn("{{item_label}}", row1_text)

        os.unlink(tmp_in.name)
        os.unlink(result_path)


class TestFillHtmlTemplate(unittest.TestCase):
    def test_item_row_without_item_stt_is_cloned(self):
        html = """
        <table>
            <tr><th>STT</th><th>Ten dich vu</th><th>Thanh tien</th></tr>
            <tr><td>1</td><td>{{item_label}}</td><td>{{item_amount}}</td></tr>
            <tr><td></td><td>Tong cong</td><td>......</td></tr>
        </table>
        """
        items = [
            {"item_stt": "1", "item_label": "Kenh MPLS", "item_qty": "1", "item_uom": "kenh", "item_price": "7,000,000", "item_amount": "7,000,000"},
        ...
        result = _fill_html_template(html, {}, items)

        self.assertIn("Kenh MPLS", result)
        self.assertNotIn("{{item_label}}", result)
        self.assertEqual(result.count("Tong cong"), 1)
