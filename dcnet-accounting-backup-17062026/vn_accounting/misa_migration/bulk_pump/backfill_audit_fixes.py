...
def post_year_end_closing(company: str) -> dict[str, Any]:
    ...
    if not batch_name:
        return {"closing_je_built": 0, "reason": "no NVK20250331 rows in any batch"}
    ...
    for idx, (acct, amts) in enumerate(legs.items()):
        ...
        je_accounts.append({
            ...
            "parent": voucher_no, "parenttype": "Journal Entry",
...
def post_remaining_nvk_jes(company: str) -> dict[str, Any]:
    ...
    pending_vnos = frappe.db.sql_list(
        """SELECT DISTINCT voucher_no FROM `tabMisa Migration Row`
    ...
    for voucher_no in pending_vnos:
        ...
        for idx, (acct, amts) in enumerate(legs.items()):
            ...
            je_accounts.append({
                ...
                "debit_in_account_currency": amts["dr"], "debit": amts["dr"],
                "credit_in_account_currency": amts["cr"], "credit": amts["cr"],
                "user_remark": remark, "parent": voucher_no,
...
@frappe.whitelist()
def run_all(company: str | None = None) -> dict[str, Any]:
