    ...
    return frappe.db.sql(
        f"""UPDATE `tabMisa Migration Row`
            SET status='Posted',
                ...
                target_name=voucher_no
            WHERE batch=%s AND voucher_no IN ({placeholders})""",
        (batch_name, *voucher_nos),
    )
...
def _update_customer_outstanding(company: str, voucher_nos: list[str]) -> int:
    ...
    if not voucher_nos:
        return 0
    # Find unique customers from inserted SI
    placeholders = ",".join(["%s"] * len(voucher_nos))
    customers = frappe.db.sql_list(
        f"SELECT DISTINCT customer FROM `tabSales Invoice` "
        f"WHERE company=%s AND name IN ({placeholders})",
        (company, *voucher_nos),
    )
    if not customers:
        return 0
...
def run_bulk_pump_full(batch_name: str, company: str = None) -> dict[str, Any]:
    ...
    ob_out = build_opening_je(batch_name, company, str(ob_date))
    if ob_out:
        _stamp_cc(ob_out)
    ...
    oi_out = build_opening_inventory(batch_name, company, str(ob_date))
    if oi_out:
        _stamp_cc(oi_out)
    ...
    for dt, vnos in by_dt.items():
        ...
        frappe.db.sql(
            f"""UPDATE `tabMisa Migration Row`
                SET status='Posted', target_doctype=%s, target_name=voucher_no
                WHERE batch=%s AND voucher_no IN ({placeholders})""",
            (dt, batch_name, *vnos),
        )
    frappe.db.commit()
    ...
    try:
        ...
        audit_summary = backfill_audit_fixes.run_all(company=company)
