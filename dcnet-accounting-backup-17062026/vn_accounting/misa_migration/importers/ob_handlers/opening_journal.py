...
def _resolve_account(
    ...
    if mapped:
        # Reject mapping entries pointing at a GROUP account — Misa
        # convention treats level-5 codes as posting leaves and ERPNext
        # rejects posting to groups. Fall through to leaf lookup.
        if frappe.db.get_value("Account", mapped, "is_group") == 0:
            return mapped
        # Mapping points at a group — log & try leaf lookup
        _log_stale_mapping_once(misa_tk, mapped)

    # 2. Direct leaf lookup by account_number on the company COA
    ...
    if live:
        # Cache the leaf hit; do NOT overwrite a working mapping (it
        # might be the same target — only update when truly stale)
