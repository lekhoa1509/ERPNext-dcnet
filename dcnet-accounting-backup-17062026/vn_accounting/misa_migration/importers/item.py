...
class ItemImporter(BaseImporter):
    ...
    def _resolve_account(self, misa_tk: str | None) -> str | None:
        """Resolve a Misa TK string to ERPNext Account.name via mapping +
        live fallback (account_number = misa_tk + company)."""
        if not misa_tk:
            return None
        misa_tk = str(misa_tk).strip()
        if not misa_tk:
            return None
        if misa_tk in self._tk_to_account:
            return self._tk_to_account[misa_tk]
        # Live fallback in case mapping is stale or missing
        if self._company:
            live = frappe.db.get_value(
                "Account", {"account_number": misa_tk, "company": self._company}, "name"
            )
            if live:
                self._tk_to_account[misa_tk] = live  # warm cache
                return live
        return None
