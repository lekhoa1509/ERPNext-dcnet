...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		# Custom Filter: only show items where safety_stock > 0 and actual_qty <= safety_stock
		if not (flt(item.safety_stock) > 0 and flt(bin.actual_qty) <= flt(item.safety_stock)):
			continue
