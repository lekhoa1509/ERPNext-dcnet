"""
Tạo Custom Fields cho Sales Order & Sales Order Item (CRM module).
Chạy: bench --site flow.local execute dcnet_apps.dcnet_apps.setup.so_custom_fields.run
"""
import frappe
...
FIELDS = {
    "Sales Order": [
        # ── Thông tin hợp đồng (sau po_date, tab chính) ────────────
        {
        ...
        # ── Tình trạng thực hiện (tab More Info, sau utm_campaign) ──
        {
...
def run():
    ...
    for dt, fields in FIELDS.items():
        data_fields = [f for f in fields if f["fieldtype"] not in ("Section Break", "Column Break")]
        print(f"  {dt}: {len(data_fields)} data fields added")
