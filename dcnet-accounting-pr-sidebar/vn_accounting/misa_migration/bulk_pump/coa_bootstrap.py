...
def bootstrap_coa(
    ...
    n_vn_numbered = frappe.db.sql(
        """SELECT COUNT(*) FROM `tabAccount`
           WHERE company=%s AND account_number REGEXP '^[0-9]'""",
        (company,),
    )[0][0]
    has_gl = bool(frappe.db.exists("GL Entry", {"company": company, "is_cancelled": 0}))
    if n_existing > 5 and n_vn_numbered == 0 and not has_gl and not force:
        # Company was created via the UI with ERPNext's Standard chart
        # (translated names, NO account numbers) and has no transactions
        # yet — a fresh-company migration target. Replace silently with
        # the VN template; asking the operator to "force" contradicts the
        # one-click pipeline and the wipe is loss-free (0 GL).
        _wipe_non_vn_coa(company)
        n_existing = frappe.db.count("Account", {"company": company})
    if n_existing > 5:
        n_after = n_existing
        ...
        # create_charts drops account_number (known ERPNext quirk) — every
        # Misa resolver keys on it. Backfill from the name's numeric prefix.
        frappe.db.sql(
            """UPDATE `tabAccount`
               SET account_number = SUBSTRING_INDEX(name, ' - ', 1)
               WHERE company=%s AND COALESCE(account_number,'')=''
                 AND name REGEXP '^[0-9]+(\\.[0-9]+)? - '""",
            (company,),
        )
        frappe.db.commit()
...
_COMPANY_DEFAULT_FIELDS = [
    ...
    # Non-party clearing leaf — the skeleton-chart ghost ("Đã nhận hàng
    # nhưng chưa xuất hóa đơn - XX") survives the Misa CoA wipe as a dead
    # pointer; PR/PI stock flows then throw. TT99: 3388 Phải trả khác.
    ("stock_received_but_not_billed", "3388"),
]
