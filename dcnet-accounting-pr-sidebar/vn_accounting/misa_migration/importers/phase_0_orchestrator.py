...
def run_phase_0_post(batch_name: str) -> dict[str, Any]:
    ...
    fa_result = post_opening_assets(batch_name, parsed.get("OB Fixed Asset", []),
                                    is_ccdc=False)
    if fa_result.get("status") in ("created", "partial"):
    ...
    ccdc_result = post_opening_assets(batch_name, parsed.get("OB CCDC", []),
                                      is_ccdc=True)
    if ccdc_result.get("status") in ("created", "partial"):
