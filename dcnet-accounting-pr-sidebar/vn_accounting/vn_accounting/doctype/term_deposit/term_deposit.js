...
frappe.ui.form.on("Term Deposit", {
	async onload(frm) {
		// Prefill default accounts from VN Accounting Settings, resolved per-company.
		if (!frm.is_new() || !frm.doc.company) return;
		const pairs = [
			["default_deposit_account", "deposit_account"],
			["default_interest_income_account", "interest_income_account"],
			["default_deposit_interest_receivable", "interest_receivable_account"],
		];
