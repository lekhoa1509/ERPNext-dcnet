...
from vn_accounting.vn_accounting.report.cash_book.cash_book import validate_filters
...
def execute(filters=None):
	...
	accounts = get_accounts_by_prefix(company, CASH_PREFIX, filters.get("cash_account"))
	if not accounts:
		return get_columns(), []
	txns = get_transactions(accounts, filters["from_date"], filters["to_date"], company)
	txns = [t for t in txns if (t.credit or 0) > 0]
	data = [
		{
			"posting_date": t.posting_date,
			"voucher_no": t.voucher_no,
			"voucher_type": t.voucher_type,
			"voucher_type_label": vn_voucher_type(t.voucher_type),
			"remarks": t.remarks,
	...
	# Sort newest first; (date, voucher_no) tuple keeps order stable.
	data.sort(key=lambda r: (r.get("posting_date") or "", r.get("voucher_no") or ""), reverse=True)
...
def get_columns():
	return [
		...
		{"label": _("Loại chứng từ"), "fieldname": "voucher_type_label", "fieldtype": "Data", "width": 140},
