 ...
 *
 * Adds "Xuất hóa đơn điện tử hàng loạt" action to the list view.
 */
...
frappe.listview_settings["Sales Invoice"].onload = function (listview) {
    ...
    // Add bulk action
    listview.page.add_action_item(__("Xuất hóa đơn điện tử hàng loạt"), () => {
        ...
        // Filter: chỉ submitted + chưa xuất HĐĐT + chưa đẩy nháp lên provider
        const valid = selected.filter(
            (row) => row.docstatus === 1 && !row.einvoice_issued && !row.einvoice_pushed
        );

        if (!valid.length) {
            frappe.msgprint("Không có Sales Invoice nào hợp lệ (phải Submitted, chưa xuất HĐ đỏ, chưa đẩy nháp).");
            return;
        }
        ...
        frappe.call({
            ...
            callback(r) {
                ...
                const d = new frappe.ui.Dialog({
                    title: `Xuất hóa đơn điện tử hàng loạt (${si_names.length} hóa đơn)`,
                    fields: [
                        {
                            fieldname: "info_html",
                            fieldtype: "HTML",
                            options: `
                                <div class="alert alert-info">
                                    Sẽ xuất <strong>${si_names.length}</strong> hóa đơn điện tử.
                    ...
                    primary_action(values) {
                        ...
                        frappe.call({
                            method: "einvoice.einvoice.api.issue_bulk_invoices",
                            args: {
...
function einvoice_show_bulk_results(data) {
    ...
    frappe.msgprint({
        title: "Kết quả xuất hóa đơn điện tử hàng loạt",
        indicator: "blue",
        message: `
            <p>${data.message}</p>
