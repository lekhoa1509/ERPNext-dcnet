(function () {
  ...
  vn_banking.init_bulk_actions = function (page, state) {
    ...
    const $draft = $(`<button class="btn btn-sm btn-primary mr-2">${__('Create draft PEs for High confidence matches')}</button>`);
    const $submit = $(`<button class="btn btn-sm btn-success">${__('Submit all draft PEs from this import')}</button>`);
