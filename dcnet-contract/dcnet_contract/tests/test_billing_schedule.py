...
class TestBillingAlignment:
	"""FB-2026-00828: per-contract billing_alignment to support kế toán's mixed billing policies."""

	def test_full_month_from_start_mid_month_first_row(self):
		"""Mid-month start under Full Month From Start: month 1 starts on acceptance day,
		not 1st of month. Amount = full monthly."""
		rows = generate_schedule(_inp(
			acceptance_date=date(2026, 3, 15),
			package_term_months=3,
			unit_price_total=Decimal("5000000"),
			billing_alignment="Full Month From Start",
		))
		assert len(rows) == 3
		assert rows[0].period_start == date(2026, 3, 15)
		assert rows[0].period_end == date(2026, 4, 14)
		assert rows[0].amount == Decimal("5000000")
		assert rows[0].is_prorated is False

	def test_full_month_from_start_no_proration_anywhere(self):
		"""Every row is full monthly amount, no row is prorated."""
		rows = generate_schedule(_inp(
			acceptance_date=date(2026, 3, 15),
			package_term_months=3,
			unit_price_total=Decimal("5000000"),
		...
		assert all(r.amount == Decimal("5000000") for r in rows)
		assert all(not r.is_prorated for r in rows)
		assert rows[2].period_start == date(2026, 5, 15)
		assert rows[2].period_end == date(2026, 6, 14)

	def test_full_month_from_start_on_first_of_month(self):
		"""Start on 1st of month: anniversary aligns to calendar, periods read like calendar months."""
		rows = generate_schedule(_inp(
			acceptance_date=date(2026, 1, 1),
			package_term_months=3,
			unit_price_total=Decimal("5000000"),
		...
		assert rows[0].period_start == date(2026, 1, 1)
		assert rows[0].period_end == date(2026, 1, 31)
		assert rows[1].period_start == date(2026, 2, 1)
		assert rows[1].period_end == date(2026, 2, 28)
		assert rows[2].period_start == date(2026, 3, 1)
		assert rows[2].period_end == date(2026, 3, 31)

	def test_manual_returns_empty(self):
		"""Manual mode: no auto-generated rows."""
		rows = generate_schedule(_inp(
			acceptance_date=date(2026, 3, 15),
			package_term_months=12,
			unit_price_total=Decimal("5000000"),
			billing_alignment="Manual",
		))
		assert rows == []

	def test_default_calendar_prorate_unchanged(self):
		"""Sanity: default mode (Calendar Prorate) still produces prorated first row."""
		rows = generate_schedule(_inp(
			acceptance_date=date(2026, 3, 15),
			package_term_months=3,
			unit_price_total=Decimal("5000000"),
		))
		# Default mode: 3 rows but first/last are prorated by day, middle is full
		assert rows[0].is_prorated is True
		assert rows[0].period_start == date(2026, 3, 15)
		assert rows[0].period_end == date(2026, 3, 31)
