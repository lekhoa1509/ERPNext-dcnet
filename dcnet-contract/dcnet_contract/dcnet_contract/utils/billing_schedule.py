...
class ScheduleInput:
	...
	# Mode A: "Calendar Prorate" (default, current behavior — partial first/last months
	# prorated by day, aligned to calendar month).
	# Mode B: "Full Month From Start" (anniversary-based periods, full monthly amount
	# every period, no prorate).
	# Mode C: "Manual" (no auto-generated rows; accountant adds ad-hoc).
	billing_alignment: str = "Calendar Prorate"
...
def generate_schedule(inp: ScheduleInput) -> list[ScheduleRow]:
	if inp.billing_alignment == "Manual":
		return []
	if inp.items:
...
def _generate_from_items(inp: ScheduleInput) -> list[ScheduleRow]:
	...
	for item in sorted(inp.items, key=_kind_order):
		...
		# Monthly recurring
		if inp.billing_alignment == "Full Month From Start":
			for k in range(1, term + 1):
				p_start = _add_months(first_start, k - 1)
				p_end = _add_months(first_start, k) - timedelta(days=1)
				rows.append(ScheduleRow(
					month_index=next_idx,
...
def _generate_legacy(inp: ScheduleInput) -> list[ScheduleRow]:
	...
	if inp.billing_alignment == "Full Month From Start":
		for idx in range(1, inp.package_term_months + 1):
			p_start = _add_months(first_start, idx - 1)
			p_end = _add_months(first_start, idx) - timedelta(days=1)
			rows.append(ScheduleRow(
				month_index=idx,
				period_start=p_start,
				period_end=p_end,
